...
let msj1 = ref("Esperando a que llenes los campos...")
let select = ref()
let horas = ref()

function revisar() {
 let total = horas.value * select.value
 msj1.value = total.toLocaleString('es-CO', { style: 'currency', currency: 'COP' });
if (horas.value >40) {
  const tarifaNormal = select.value;
const horasTrabajadas = horas.value;
...
msj1.value = "La respuesta es" + " " + salario.toLocaleString('es-CO', { style: 'currency', currency: 'COP' });
}
}
...
<div style="margin: auto; border: gray; height: 400px; width: 600px; margin-top: 10%; background-color: #d1d8e0; border-radius: 10px;" >

  <br>

  <div style="height: 30px; width: 550px; margin-left: 20px; " class="input-group mb-3">
  <label  class="input-group-text" for="inputGroupSelect01">Category</label>
  <select v-model="select" class="form-select" id="inputGroupSelect01">
    
    <option value="12000">one</option>
    <option value="17000">Two</option>
    <option value="22000">Three</option>
  </select>
</div>

<br>

<div  style="height: 40px; width: 550px; margin-left: 20px; " class="input-group flex-nowrap">
  <span  class="input-group-text" id="addon-wrapping">Hours</span>
  <input v-model="horas" type="number" class="form-control"  placeholder="Ingresa las horas que trabajaste" aria-label="Username" aria-describedby="addon-wrapping">
</div>

<br><br>

<center>
  <button @click="revisar()" >
...
<div style="text-align: center;" >{{ msj1 }}</div>
